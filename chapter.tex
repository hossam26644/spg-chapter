...
The
\texttt{sim_ancestry} function behaves slightly differently when it is
...
\texttt{sim_ancestry} were the \texttt{sample\_size} and
...
\texttt{PopulationConfiguration\ objects}, the \texttt{population_size} parameter to
\texttt{sim_ancestry} is not required, as the \texttt{initial\_size} of the
...
parameters in the \texttt{sim_ancestry} function. The list of population
...
being allocated sequentially to the populations when \texttt{sim_ancestry}
...
supplied to \texttt{sim_ancestry} to turn on this feature.) Migration
...
then supply this object to \texttt{sim_ancestry} using the
