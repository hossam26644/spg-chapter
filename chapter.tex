...
% \qquad
\subfloat[][Population split]{
\includegraphics[width=0.3\textwidth]{images/population_split.pdf}
\label{fig-trees-population-split}}
% \qquad
\subfloat[][Admixture]{
\includegraphics[width=0.3\textwidth]{images/admixture.pdf}
\label{fig-trees-admixture}}
\caption{\label{fig-trees-demographic-events} Examples of different demographic events
that could be simulated by \msprime.}
\end{figure}

\runinhead{Population split}\label{population-split} \

To simulate a scenario where a population is derived from a specific population at a certain time (split event), the
\texttt{add\_population\_split()} method of a \texttt{Demography} object can be used. This method takes the time of the split event,
the source population, and the destination population as arguments. The following code chunk demonstrates how to
simulate population split events:
...
demography.add_population_split(time=10,
                                derived=["A", "B"],
                                ancestral="AB")
...
This code chunk encodes the tree in figure~\ref{fig-trees-population-split}.

\runinhead{Admixture}\label{admixture} \

To simulate a population admixture event, where a population is formed by the admixture of two or more populations,
the \texttt{Demography.add\_admixture()} method can be used. This method takes the time of the admixture event,
the source populations, the destination population, and the proportion of each source population as arguments. The
following code chunk demonstrates how to simulate the population admixture event illustrated in figure~\ref{fig-trees-admixture}:
\begin{pythoncode}
    demography = msprime.Demography()
    demography.add_population(name="A", initial_size=100)
    demography.add_population(name="B", initial_size=100)
...
This code chunk simulates an ancestral population, "ANC", that splits into populations A and B at time 20.
At time 10, populations A and B admix to form population ADMIX. The proportions of the admixture are 0.25 and 0.75,
respectively.
